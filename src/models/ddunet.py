...
class DDUNet(nn.Module):
    ...
    def __init__(self, args, num_input_channels=2, num_output_channels=2,
        ...
        elif args.dilation_type == 'exponential':
        ...
        self.start_transition = dconv(num_input_channels+2*filters[0], filters[0], 1, bias=need_bias, pad=pad, dilation=self.transition_dilation)

        self.down1 = unetDown(filters[0], filters[1], norm_layer, need_bias, pad, self.dilation[1])
        self.down1_transition = dconv(filters[0]+2*filters[1], filters[1], 1, bias=need_bias, pad=pad, dilation=self.transition_dilation)

        self.down2 = unetDown(filters[1], filters[2], norm_layer, need_bias, pad, self.dilation[2])
        self.down2_transition = dconv(filters[1]+2*filters[2], filters[2], 1, bias=need_bias, pad=pad, dilation=self.transition_dilation)
        ...
        self.up2_transition = dconv(filters[0]+filters[1]+2*filters[2], filters[1], 1, bias=need_bias, pad=pad, dilation=self.transition_dilation)

        self.up1 = unetUp(filters[0], num_input_channels, upsample_mode, need_bias, pad, self.dilation[4])
        self.up1_transition = dconv(num_input_channels+filters[0]+2*filters[1], filters[0], 1, bias=need_bias, pad=pad, dilation=self.transition_dilation)
        self.final = dconv(filters[0], num_output_channels, 1, bias=need_bias, pad=pad, dilation=self.dilation[5])
...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, norm_layer, need_bias, pad, dilation):
        ...
        if norm_layer is not None:
            self.conv1= nn.Sequential(dconv(in_size, out_size, 3, bias=need_bias, pad=pad, dilation=dilation[0]),
                                       norm_layer(out_size),
                                       nn.ReLU(),)
            self.conv2= nn.Sequential(dconv(in_size+out_size, out_size, 3, bias=need_bias, pad=pad, dilation=dilation[1]),
                                       norm_layer(out_size),
                                       nn.ReLU(),)
        else:
            self.conv1= nn.Sequential(dconv(in_size, out_size, 3, bias=need_bias, pad=pad, dilation=dilation[0]),
    ...
    def __init__(self, out_size, init_filters, upsample_mode, need_bias, pad, dilation=[2], same_num_filt=False):
        ...
        if upsample_mode == 'deconv':
            ...
            self.up = nn.Sequential(nn.Upsample(scale_factor=2, mode=upsample_mode),
                                   dconv(num_filt, out_size, 3, bias=need_bias, pad=pad, dilation=1))
