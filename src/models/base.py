...
def conv(type, in_f, out_f, kernel_size, stride=1, dilation=1, bias=True, anchor=1):
    """
        The convolutional filter with same zero pad.
    """
    if isinstance(kernel_size, list):
        to_pad = (int((kernel_size[0] - 1)/2), int((kernel_size[1] - 1)/2))
        if type=='hconv':
            to_pad = to_pad[1]
    else:
    ...
    if type=='hconv':
        convolver = harmonic_conv.SingleHarmonicConv2d(in_f, out_f, kernel_size, anchor=anchor, stride=stride, dilation=dilation, padding=(0,to_pad), bias=bias, padding_mode="zeros")
    else:
        convolver = nn.Conv2d(in_f, out_f, kernel_size, stride, dilation=dilation, padding=to_pad, bias=bias)
...
def conv2dbn(type, in_f, out_f, kernel_size, stride=1, dilation=1, bias=True, act_fun='LeakyReLU', anchor=1):
    block = conv(type, in_f, out_f, kernel_size, stride=stride, dilation=dilation, bias=bias, anchor=anchor)
